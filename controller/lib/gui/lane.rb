...
module Schem
  ...
  class HexWidgetLane
    ...
    def repr(text,length=1,tags = [], classes = [])
        tags = tags.map do |t|
          case t
            ...
            else raise "expected a tag instead of #{t.inspect}"
          end
        end
    ...
    def perform_action(name,clicked,selected_range)
      synchronize do
        ...
        raise "unknown action #{name.inspect} for #{self.class}" unless action
    ...
    def get_line_reprs(address_range)
      ...
      return [some_repr, some_other_repr]
    end
    ...
    def render_colgroup
      return "<col span='#{self.class.get_colspan}' #{self.class.get_colstyle}/>"
    end
    ...
    def render_line(address_range)
      synchronize do
        ...
        if a - address_range.min != self.class.get_colspan
          raise "missmatch number of colspans #{a-address_range.min} != #{self.class.get_colspan.inspect} from #{self.class}"
        end
        return line
      end
    end
    ...
    def self.colspan(x); @colspan = x end
    def self.get_colspan; @colspan or raise "colspan not initialized for #{self}" end
    def self.colstyle(x); @colstyle = x end
    def self.get_colstyle; @colstyle ||= "" end
  ...
  class AddressLane < HexWidgetLane
    ...
    action "add label" do |clicked, selected|
    ...
    action "relative address" do |clicked_range,selected_range|
    ...
    action "absolute address" do |clicked_range,selected_range|
      @relative_target = nil
      update!
    end
    ...
    def get_line_reprs(address_range)
      ...
      tags = []
      return [repr(repr, 1, tags, classes)]
    end
  end
