...
module Schem
  module GdbDebuggerApi
    ...
    def internal_list_shared_objects
      max_tries(3, 0.01, 'gdb_api:internal_list_shared_objects') do
    ...
    def internal_bp_create_helper(address, mode)
      max_tries(3, 0.01, 'gdb_api:internal_bp_create_helper') do
        case mode.to_s
        ...
        assert('gdb was unable to add a breakpoint') { res.content_type == 'done' }
    ...
    def internal_wp_create_helper(address, mode)
      mode, res = mode.to_s, ''
      max_tries(3, 0.01, 'gdb_api:internal_wp_create_helper') do
    ...
    def internal_bp_delete(bp)
      ...
      max_tries(3, 0.01, 'gdb_api:internal_bp_delete') do
    ...
    # raise "unable to extract breakpoint list" if gdb failed to give us the bplist after 3 tries
    def internal_bp_list
      max_tries(3, 0.01, 'gdb_api:internal_bp_list') do
        ...
        assert('gdb did not return a breakpoint list') { !bplist.nil? }
    ...
    # checks that the register length didn't change (gdb sometimes sends to few registers)
    # @param
    # @return
    # raise
    ...
    def internal_get_register_names
      max_tries(3, 0.01, 'gdb_api:internal_get_register_names') do
        ...
        # And because gdb is  ŝ̜̟̜͇ͨͧ͋h̴̻̘̩͙̪͔ͧͯͤͦͯ̚ị̡̘̜̼̃̃ͨtͬ̈́
    ...
    # raise 'unable to set register...' if gdb was unable to set the register
    # to the value
    def internal_set_register(name, value)
      ...
      reg_number = regs[name].value['number']
      max_tries(3, 0.01, 'gdb_api:internal_set_register') do
    ...
    def internal_mem_read(address, size)
      ...
      max_tries(3, 0.01, 'gdb_api:internal_mem_read') do
        ...
        assert('gdb returned a wrong amount of bytes') { res.length == size }
        return res
      end
    end
    ...
    def internal_mem_write(address, contents)
      ...
      max_tries(3, 0.01, 'gdb_api:internal_mem_write') do
    ...
    def internal_get_mapped_images
      ...
      if old_shared_objects == internal_list_shared_objects && !@mapped_images.empty?
        ...
        max_tries(3, 0.01, 'gdb_api:internal_get_mapped_images') do
    ...
    def internal_get_image_bin(path)
      # TODO if not present locally get it via gdb
      File.read(path)
    end
    ...
    def internal_mem_mappings
      max_tries(3, 0.01, 'gdb_api:internal_mem_mappings') do
        res = @debugger.send_cli_string('info proc mappings', /\Aprocess\s+\d+\\n\Z/)
        assert { res.is_a?(String) && res != '' }
        matches = res.split('\n').map do |x|
          /(?<start>0x[a-fA-F0-9]+)\s+(?<end>0x[a-fA-F0-9]+)\s+(?<size>0x[a-fA-F0-9]+)\s+(?<offset>0x[a-fA-F0-9]+)\s+(?<object_file>[\/\[].*)/.match(x)
        end
        assert('no sections mapped') { matches.length > 0 }
        sections = matches.compact.map do |x|
          from = x['start'].to_gdbi
          to = x['end'].to_gdbi - 1
          length = x['size'].to_gdbi
          object_file = x['object_file']
          offset = x['offset']
    ...
    def internal_step_over
      max_tries(3, 0.01, 'gdb_api:step_over') do
    ...
    def internal_step_into
      max_tries(3, 0.01, 'gdb_api:internal_step_into') do
