...
module Schem
  ...
  class FileImage
    ...
    def get_section_name(section)
      return "section:bit:"+Digest::SHA512.hexdigest(section[:data])
    end
    ...
    def rva_to_va(rva)
      ...
      return rva #executables use rva in metasm
    end

    def va_to_rva(va)
      return va-@range.min if shared_object?
      return rva-0x400000 #TODO WTF WTF WTF this is so fucking broken
    end
  ...
  class ObjectService < BaseService
    ...
    def get_image(address)
      imageid,range = @images.each_pair.find{|imageid,range| range.include? address}
      return nil unless range
      return @images_by_id[imageid]
    end
      ...
      if images_to_ranges.keys != @images_to_ranges.keys
        (images_to_ranges.keys-@images_to_ranges.keys).each do |imgid|
          ...
          if images_to_ranges.each_pair.any? { |img,irange| (range.intersection(irange) && img != imgid ) }
            Log.error("colliding images") and next
          end
