...
module Schem
  ...
  class ConversionService < BaseService
    ...
    def parse_signed(mem)
      ...
      fail "Cannot parse #{length} bit integer" unless format
    ...
    def dump_unsigned(value, length)
      format = UNSIGNED[length]
      fail "Cannot dump #{length} bit integer" unless format
      [value].pack(format)
    end
    ...
    def repr(value, reg_length)
      return unless value
      { hex: to_hex(value, reg_length), signed: to_signed(value, reg_length), unsigned: to_unsigned(value, reg_length) }
    end
  end
