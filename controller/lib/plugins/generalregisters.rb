  ...
  class GeneralRegistersPlugin < StructViewPlugin
    ...
    action "edit" do |name|
      ...
      if width == 32
        ...
        raise "word width #{width} not supported"
      end
      repr = srv.int.repr(reg_value, reg_length)
      answer_value = srv.dialog.prompt("Edit: #{name.to_s}\nhex: #{repr[:hex]}\nsigned:#{repr[:signed]}\nunsigned: #{repr[:unsigned]}", repr[1])["answer"]
    ...
    def remove_unset(registers)
      registers = registers.map_values{|k1,v1| v1.delete_if { |k, v| v == "nil" || v == nil }}
      registers = registers.delete_if { |k,v| v.empty? }
