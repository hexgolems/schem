module Schem
  ...
  class InferredType
    ...
    def ==(other)
      ...
      return false unless continued == other.continued
      return false unless signed == other.signed
      return false unless struct == other.struct
      return false unless byte == other.byte
      true
    end
  ...
  class TypeInformationBitmap
    ...
    def initialize(name, _img, range, srv)
      @srv = srv
      @name = name
      @length = range.size
      @range = range
    ...
    def inspect
      '<class: ' + self.class.to_s + ', name: ' + @name.to_s + ', length: ' + @length.to_s + ' , range:' + @range.min.to_s + '..' + @range.max.to_s + ' >'
    end
    ...
    def type_string(type, length = 0)
      ...
      fail "unknown type #{type.inspect}" unless string
      (length - 1).times { string += FROM_TYPES[(type.to_s + '_continued').to_sym] }
      string
    end

    def to_rva(address)
      unless @range.include?(address)
        fail "address #{address.to_s 16} not in range #{@range.hex_inspect}"
      end
      address - @range.min
    end
    ...
    def get_type_string(range)
      fail 'invalid range' unless range.intersection @range
      overflow_top = ''
      overflow_bottom = ''
      ...
      overflow_bottom + bit_get_range(range_rva) + overflow_top
    end

    def bit_get_range(rva_range)
      @srv.db[@name][rva_range.min..rva_range.max]
    end
    ...
    def set_type_string(address, string)
      ...
      fail 'invalid range' unless @range.include?(range.min) && @range.include?(range.max)
    ...
    def get_prefix_byte_types(address)
      ...
      loop do
        ...
        types.unshift type
        return types unless type.continued
      end
    end
